...
### To see the app live online, you need to connect RStudio with ShinyApp.
# Then, you can press the publish button (top right blue circle button in R studio)
# to publish it. The page is hosted at: 
# https://qaqc-miniapp.shinyapps.io/chrl-graph/
  ...
  dashboardPage(title = "CHRL Real-time Weather",
                ...
                dashboardSidebar(
                  ...
                  sidebarMenu(id = "smenu",
                              ...
                              menuItem("Custom Graphs", tabName = "cstm_graph", icon = icon("fas fa-chart-line")) # normally  add comma here
                              # menuItem("Annual Comparisons", tabName = "ann_compare", icon = icon("fas fa-chart-line")),
                              # menuItem("QAQC Graphs", tabName = "qaqc_graph", icon = icon("fas fa-chart-line")),
                ...
                dashboardBody(
                  ...
                  tabItems(
                    ...
                    tabItem("qaqc_graph",
                            fluidRow(
                              column(12,
                                     h1("QAQC Graphs", align = "center")
                              )
                            ),
                            fluidRow(
                              column(2,
                                     selectInput("qaqc_site",
                                                 label = "Choose a Weather Station:",
                                                 choices = stnNameDict,
                                                 selected = cur_stn,
                                                 selectize = F
                                     ...
                                     uiOutput("varSelection_qaqc"),
                                     #uiOutput("cleanSnowButton_qaqc"),
                                     selectInput("qaqc_year", "Select Water Year", "",  selectize = F)
                              ),
                              column(10,
                                     htmlOutput('header4'),
                                     wellPanel(
                                       plotlyOutput("plot5", height = "40vh"),
                                     ),
                                     htmlOutput('partnerLogoUI_qaqc')
                              )
...
server <- function(input, output, session) {
  ...
  # source("classes/qaqc.r", local = TRUE)
